...
internal class AggregateTest(
    @Autowired val aggregate: StateStoredAggregate<Either<RestaurantOrderCommand?, RestaurantCommand?>, Pair<RestaurantOrder?, Restaurant?>, Either<RestaurantOrderEvent?, RestaurantEvent?>>
) : StateRepository<Either<RestaurantOrderCommand?, RestaurantCommand?>, Pair<RestaurantOrder?, Restaurant?>> by aggregate {
    ...
    fun `Assert handling CreateRestaurantCommand command is successful`() = runBlocking<Unit> {
        ...
        assertThat(aggregate.handle(Either.Right(createRestaurantCommand)).isRight()).isTrue
        ...
        val actualResult = Either.Right(createRestaurantCommand).fetchState()
    ...
    fun `Assert handling createRestaurantOrderCommand is successful`() = runBlocking<Unit> {
        ...
        assertThat(aggregate.handle(Either.Left(createRestaurantOrderCommand)).isRight()).isTrue
        ...
        val actualResult = Either.Left(createRestaurantOrderCommand).fetchState()
        assertThat(actualResult.isRight()).isTrue
    ...
    fun `Assert handling commands is successful`() = runBlocking<Unit> {
        ...
        assertThat(aggregate.handle(Either.Left(markRestaurantOrderAsPreparedCommand)).isRight()).isTrue
        ...
        val actualResult2 = Either.Left(markRestaurantOrderAsPreparedCommand).fetchState()
